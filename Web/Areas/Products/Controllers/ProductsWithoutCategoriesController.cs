...
{
    public class ProductsWithoutCategoriesController : AuthorizeController
    {
        [HttpGet]
        public ActionResult List()
        {
            ProductsWithoutCategoriesList vm = ProductsWithoutCategoriesHelper.CreateModel();
            SaveModelToCache(vm);
        ...
        [HttpPost]
        public ActionResult List(ProductsWithoutCategoriesList vm)
        {
            ProductsWithoutCategoriesList cached = GetModelFromCache<ProductsWithoutCategoriesList>(vm.PageID);
            ProductsWithoutCategoriesHelper.UpdateModel(vm, cached, IsExporting);
        ...
        public ActionResult GrdMainCallback(string pageID)
        {
            ...
            ProductsWithoutCategoriesList vm = GetModelFromCache<ProductsWithoutCategoriesList>(pageID);
            ...
            if (args.Count == 2)
            {
                ...
                if (refresh)    // We may have manually removed some rows from the grid data so no need to go to DB
                {
                    ...
                    vm.Grids[name].Data = ProductsWithoutCategoriesHelper.GetData(vm);
                    SaveModelToCache(vm);
                }
            }

            // If sorting and we have max rows then we need to go back to the DB
            var data = (IEnumerable<ProductsWithoutCategoriesInfo>)vm.Grids[name].Data;
        ...
        [AjaxOnly]
        public ActionResult Assign(string pageID, int categoryKey, string ids)
        {
            ProductsWithoutCategoriesList vm = GetModelFromCache<ProductsWithoutCategoriesList>(pageID);

            Exception ex = ProductsWithoutCategoriesHelper.Assign(vm, categoryKey, ids);
